...
class SantaAgent:
    ...
    y = 0  # type: int
    heading = "N"  # type: str
    history = []  # type: list[(int, int)]
    found_hq = False  # type: bool
    ...
    def move(self, step: str):
        ...
        for step in range(0, distance):
            self.x += self.multipliers[self.heading][0]
            self.y += self.multipliers[self.heading][1]
    ...
    while not should_quit(raw_directions):
        ...
        if not should_quit(raw_directions):
            ...
            for direction in directions:
                ...
                if agent.found_hq:
                    break
            print("EBH is at ({}, {}). Distance between LZ and EBH is {}".format(agent.x, agent.y,
